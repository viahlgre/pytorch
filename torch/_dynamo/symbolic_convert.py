...
class InstructionTranslatorBase(
    ...
    def _maybe_sync_dealloc_attr(self, obj: VariableTracker, name: str) -> None:
        ...
        # Check side_effects first — pure dict lookup, no side effects.
        ...
        # For input containers, try var_getattr. Skip for UDOVs with custom
        # __getattr__/__getattribute__ to avoid executing user code.
        if isinstance(obj, UserDefinedObjectVariable):
            if obj._object_has_getattribute or obj._check_for_getattr():
                return

        try:
            attr_var = obj.var_getattr(self, name)  # type: ignore[arg-type]
            if isinstance(attr_var, TensorVariable):
                self._maybe_emit_sync_dealloc(attr_var)
        except Exception:
            pass
